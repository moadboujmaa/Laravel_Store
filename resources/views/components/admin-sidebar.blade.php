   ...
   <aside id="default-sidebar" class="top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
      <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
         <ul class="space-y-2 font-medium">
            <li 
               @if (explode('/', url()->full())[4] == 'dashboard')
            ...
            <li 
               @if (explode('/', url()->full())[4] == 'users')
                  class='rounded-md bg-slate-600'
               @endif
            >
               <a href={{ route('admin.users.show') }} class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            <li
               @if (explode('/', url()->full())[4] == 'categories')
                  class='rounded-md bg-slate-600'
               @endif
            >
               <a href={{ route('admin.categories.index') }} class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            <li 
               @if (explode('/', url()->full())[4] == 'products')
                  class='rounded-md bg-slate-600'
               @endif
            >
            ...
            <li
               @if (explode('/', url()->full())[4] == 'orders')
                  class='rounded-md bg-slate-600'
               @endif
            >
               <a href={{ route('admin.order.index') }} class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                  <svg aria-hidden="true" class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            ...
            <li>
               <a href={{ route('home') }}
                  class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                  <img src={{ asset('icons/move-left.png') }} class="w-6" alt="logo">
